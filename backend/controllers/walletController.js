...
function transactionSign(type) {
  const creditTypes = ['Top Up', 'Top-Up', 'Refund', 'Credit'];
...
async function topUp(req, res) {
  try {
    ...
    await walletModel.topUpWallet(userId, req.body.amount);
...
async function purchasePass(req, res) {
  try {
    ...
    const result = await walletModel.purchasePass(userId, req.body.passType);
...
async function showHistory(req, res, next) {
  try {
    ...
    const transactions = await walletModel.getTransactionHistory(userId);
...
async function showAdminDashboard(req, res, next) {
  try {
    const dashboard = await adminWalletModel.getAdminWalletDashboard();
