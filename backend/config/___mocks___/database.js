...
function reset() {
    store = {
        users: [
            { id: 1, name: 'Administrator', email: 'admin@bikeapp.com', role: 'Admin' },
            { id: 2, name: 'Member User', email: 'member@bikeapp.com', role: 'Member' },
...
async function execute(sql, params = []) {
    ...
    if (q.includes('UPDATE bikes') && q.includes('SET') && q.includes('name = ?')) {
    ...
    if (q.includes('UPDATE wallets') && q.includes('tripCredits = ?') && q.includes('dayPassCredits = ?') && q.includes('balance = ?')) {
    ...
    if (q.includes('INSERT INTO wallet_transactions') && q.includes('SELECT')) {
        ...
        store.wallet_transactions.push({
            transactionId: id, userId: Number(userId), type, amount, balanceAfter,
    ...
    if (q.includes('FROM wallet_transactions') && q.includes('WHERE userId = ?')) {
        const [userId] = params;
    ...
    if (q.includes('INSERT INTO rentals')) {
        const [userId, userName, bikeId, bikeName, amount, paymentMethod, status] = params;
        const id = store.nextRentalId++;
        store.rentals.push({
            id, userId, userName, bikeId, bikeName, amount, paymentMethod,
            status, rentedAt: new Date().toISOString(), returnedAt: null,
        });
        return [{ insertId: id, affectedRows: 1 }];
    }
    ...
    if (q.includes('SELECT *') && q.includes('FROM rentals') && q.includes('WHERE id = ?')) {
        const [id] = params;
        const r = findRental(id);
        return [r ? [{ ...r }] : []];
    }
    ...
    if (q.includes('FROM rentals') && q.includes('bikeName IS NOT NULL')) {
        ...
        const rows = store.rentals
            ...
            .sort((a, b) => new Date(b.returnedAt || b.rentedAt) - new Date(a.returnedAt || a.rentedAt));
        return [rows.length ? [rows[0]] : []];
    }
    if (q.includes('FROM rentals') && q.includes('WHERE userId = ?')) {
        const [userId] = params;
        const rows = store.rentals.filter((r) => Number(r.userId) === Number(userId));
        return [rows];
    }
    if (q.includes('FROM rentals') && q.includes('ORDER BY rentedAt')) {
        return [[...store.rentals].sort((a, b) => new Date(b.rentedAt) - new Date(a.rentedAt))];
    }
    ...
    if (q.includes('INSERT INTO tracker')) {
        ...
        if (!t) {
            t = { userId: Number(userId), userName, city, bikeName, distance: Number(distance), rides: 1, lastRideAt: new Date().toISOString() };
    ...
    if (q.includes('FROM tracker')) {
        return [[...store.tracker].sort((a, b) => b.distance - a.distance || b.rides - a.rides)];
    }

    // ============ LEADERBOARD ============
    if (q.includes('INSERT INTO leaderboard')) {
        const [userId, userName, city, bikeName, distance, rides, lastRideAt] = params;
        let l = store.leaderboard.find((x) => Number(x.userId) === Number(userId));
        if (!l) {
            l = { userId: Number(userId), userName, city, bikeName, distance, rides, lastRideAt };
            store.leaderboard.push(l);
        } else {
            Object.assign(l, { userName, city, bikeName, distance, rides, lastRideAt });
        }
        return [{ affectedRows: 1 }];
    }
    if (q.includes('FROM leaderboard')) {
        return [[...store.leaderboard].sort((a, b) => b.distance - a.distance || b.rides - a.rides)];
    }
    ...
    if (q.includes('INSERT INTO bike_repair_reports')) {
        ...
        store.bike_repair_reports.push({
            id, bikeStation, bikeID, issueType, description, status: 'Pending', date: reportDate,
        });
        return [{ insertId: id, affectedRows: 1 }];
    }
    if (q.includes('UPDATE bike_repair_reports')) {
        const [status, id] = params;
        const r = store.bike_repair_reports.find((x) => Number(x.id) === Number(id));
        if (!r) return [{ affectedRows: 0 }];
        r.status = status;
        return [{ affectedRows: 1 }];
    }
    ...
    if (q.includes('INSERT INTO rewards')) {
        ...
        if (!store.rewards.find((r) => Number(r.userId) === Number(userId))) {
            store.rewards.push({
                userId: Number(userId), points: 0, freeHours: 0, rides: 0,
                totalMinutes: 0, referralCode, friendsReferred: 0, referredBy: null,
            });
        }
        return [{ affectedRows: 1 }];
    }
    if (q.includes('UPDATE rewards') && q.includes('rides = rides + 1')) {
        ...
        r.points += points; r.totalMinutes += minutes; r.rides += 1;
        return [{ affectedRows: 1 }];
    }
    if (q.includes('UPDATE rewards') && q.includes('friendsReferred = friendsReferred + 1')) {
        const [points, userId] = params;
        const r = store.rewards.find((x) => Number(x.userId) === Number(userId));
        r.friendsReferred += 1; r.points += points;
        return [{ affectedRows: 1 }];
    }
    if (q.includes('UPDATE rewards') && q.includes('referredBy = ?')) {
        const [code, userId] = params;
        const r = store.rewards.find((x) => Number(x.userId) === Number(userId));
        r.referredBy = code; r.points += 5;
        return [{ affectedRows: 1 }];
    }
    if (q.includes('UPDATE rewards') && q.includes('freeHours = freeHours + ?')) {
        const [points, hours, userId] = params;
        const r = store.rewards.find((x) => Number(x.userId) === Number(userId));
        r.points -= points; r.freeHours += hours;
        return [{ affectedRows: 1 }];
    }
    if (q.includes('FROM rewards') && q.includes('INNER JOIN users')) {
        const [code] = params;
        const r = store.rewards.find((row) => String(row.referralCode).toUpperCase() === String(code).toUpperCase());
        if (!r) return [[]];
    ...
    if (q.includes('INSERT INTO reward_transactions')) {
        ...
        if (q.includes("VALUES (?, 5, 'earned', ?)")) {
            [userId, description] = params; points = 5; type = 'earned';
        } else if (q.includes("'earned'")) {
            [userId, points, description] = params; type = 'earned';
        } else if (q.includes("'redeemed'")) {
            [userId, points, description] = params; type = 'redeemed';
        } else {
            [userId, points, type, description] = params;
        }
        store.reward_transactions.push({
            id, userId: Number(userId), points: Number(points), type, desc: description,
