...
describe('getOrCreateWallet', () => {
  ...
  test('rejects an invalid user id', async () => {
    await expect(walletModel.getOrCreateWallet(-1)).rejects.toThrow(
      /invalid user id/i
    );
    await expect(walletModel.getOrCreateWallet('abc')).rejects.toThrow(
      /invalid user id/i
    );
  });
...
describe('topUpWallet', () => {
  test('rejects a top-up of 0 or less', async () => {
    ...
    await expect(walletModel.topUpWallet(2, -10)).rejects.toThrow(
      /more than 0/i
    );
  });
...
describe('purchasePass', () => {
  test('rejects an unknown pass type', async () => {
    ...
    await expect(
      walletModel.purchasePass(2, 'not_a_real_pass')
    ).rejects.toThrow(/valid pass/i);
  });

  test('throws InsufficientBalanceError and logs a Failed transaction when balance is too low', async () => {
    await expect(walletModel.purchasePass(2, 'day_pass')).rejects.toThrow(
      walletModel.InsufficientBalanceError
    );
    const history = await walletModel.getTransactionHistory(2);
    expect(history).toHaveLength(1);
