...
async function seedBikeAndWallet({ price = 5, balance = 0, tripCredits = 0, dayPassCredits = 0 } = {}) {
    const bike = await bikeModel.addBike({ name: 'City Bike', description: '', price });
...
describe('rentBike payment priority', () => {
    test('throws when the bike does not exist', async () => {
        await walletModel.getOrCreateWallet(2);
        await expect(rentalModel.rentBike({ userId: 2, userName: 'x', bikeId: 999 }))
    ...
    test('throws when the bike is already rented', async () => {
        ...
        await expect(rentalModel.rentBike({ userId: 2, userName: 'x', bikeId: bike.id }))
            .rejects.toThrow(/already been rented/i);
    });

    test('throws when the wallet does not exist', async () => {
        const bike = await bikeModel.addBike({ name: 'City Bike', description: '', price: 5 });
        await expect(rentalModel.rentBike({ userId: 999, userName: 'x', bikeId: bike.id }))
    ...
    test('day pass credits are used first, even with trip credits and balance available', async () => {
        const bike = await seedBikeAndWallet({ price: 5, balance: 100, tripCredits: 2, dayPassCredits: 3 });
        const rental = await rentalModel.rentBike({ userId: 2, userName: 'x', bikeId: bike.id });
    ...
    test('trip credits are used second, when there is no day pass credit', async () => {
        const bike = await seedBikeAndWallet({ price: 5, balance: 100, tripCredits: 2, dayPassCredits: 0 });
        const rental = await rentalModel.rentBike({ userId: 2, userName: 'x', bikeId: bike.id });
...
describe('getUserRentals', () => {
    test('only returns rentals belonging to the requested user', async () => {
        ...
        Object.assign(db._store().wallets.find((w) => w.userId === 3), { balance: 100 });
        const bike2 = await bikeModel.addBike({ name: 'Bike 2', description: '', price: 3 });
