...
describe('getAllBikes / getBikeById', () => {
  ...
  test('getBikeById finds a bike by id', async () => {
    const created = await bikeModel.addBike({
      name: 'Bike A',
      description: '',
      price: 5,
    });
...
describe('addBike', () => {
  test('assigns sequential ids starting from 1', async () => {
    const first = await bikeModel.addBike({
      name: 'Bike A',
      description: '',
      price: 5,
    });
    const second = await bikeModel.addBike({
      name: 'Bike B',
      description: '',
      price: 8,
    });
  ...
  test('rejects a missing name', async () => {
    await expect(
      bikeModel.addBike({ name: '  ', description: '', price: 5 })
    ).rejects.toThrow(/name is required/i);
  });

  test('rejects an invalid price', async () => {
    await expect(
      bikeModel.addBike({ name: 'Bike A', description: '', price: -5 })
...
describe('updateBikeStatus', () => {
  test('rejects an invalid status', async () => {
    ...
    await expect(bikeModel.updateBikeStatus(bike.id, 'Broken')).rejects.toThrow(
      /invalid bike status/i
    );
  });
