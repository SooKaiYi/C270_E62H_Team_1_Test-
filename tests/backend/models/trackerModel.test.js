...
describe('saveRideDistance', () => {
  test('rejects an invalid user id', async () => {
    await expect(trackerModel.saveRideDistance({ id: -1 }, 5)).rejects.toThrow(
      /invalid user id/i
    );
  });

  test('rejects a negative distance', async () => {
    await expect(trackerModel.saveRideDistance({ id: 2 }, -5)).rejects.toThrow(
      /invalid distance/i
    );
  });
  ...
  test('creates a new tracker entry on the first ride', async () => {
    const tracker = await trackerModel.saveRideDistance(
      { id: 2, name: 'Member User', city: 'Singapore' },
      3.5,
      'City Bike'
    );
  ...
  test('accumulates distance and increments rides on subsequent rides', async () => {
    await trackerModel.saveRideDistance(
      { id: 2, name: 'Member User' },
      3.5,
      'City Bike'
    );
    const second = await trackerModel.saveRideDistance(
      { id: 2, name: 'Member User' },
      2.5,
      'City Bike'
    );
