...
describe('submitRepairReport', () => {
    ...
    test('successfully creates a report with status Pending', async () => {
        const req = {
            body: { bikeStation: 'Station A', bikeID: 'B123', issueType: 'Flat Tire', description: 'Rear tire flat' },
            session: {},
        };
        const res = mockRes();
        await repairController.submitRepairReport(req, res);
        ...
        expect(res.data.report).toMatchObject({
            bikeStation: 'Station A', bikeID: 'B123', issueType: 'Flat Tire', status: 'Pending',
        });
    });

    test('trims whitespace from all text fields', async () => {
        const req = {
            body: { bikeStation: '  Station A  ', bikeID: ' B123 ', issueType: ' Flat Tire ', description: '  ' },
            session: {},
        };
        const res = mockRes();
        await repairController.submitRepairReport(req, res);
...
describe('showAdminPage', () => {
    ...
    test('shows all reports to an admin, newest first', async () => {
        await repairController.submitRepairReport(
            { body: { bikeStation: 'A', bikeID: '1', issueType: 'Flat' }, session: {} }, mockRes()
        );
        await repairController.submitRepairReport(
            { body: { bikeStation: 'B', bikeID: '2', issueType: 'Brake' }, session: {} }, mockRes()
        );
...
describe('updateRepairStatus', () => {
    test('rejects an invalid status value', async () => {
        const req = { params: { id: 1 }, body: { status: 'Deleted' }, session: { user: { role: 'Admin' } } };
        const res = mockRes();
    ...
    test('rejects non-admin users', async () => {
        const req = { params: { id: 1 }, body: { status: 'Resolved' }, session: { user: { role: 'Member' } } };
        const res = mockRes();
        await repairController.updateRepairStatus(req, res);
        expect(res.statusCode).toBe(403);
    });
    ...
    test('returns 404 for a report that does not exist', async () => {
        const req = { params: { id: 999 }, body: { status: 'Resolved' }, session: { user: { role: 'Admin' } } };
        const res = mockRes();
        await repairController.updateRepairStatus(req, res);
    ...
    test('successfully updates the status and redirects', async () => {
        ...
        const req = { params: { id: 1 }, body: { status: 'Resolved' }, session: { user: { role: 'Admin' } } };
        const res = mockRes();
        await repairController.updateRepairStatus(req, res);
        ...
        await repairController.showAdminPage({ session: { user: { role: 'Admin' } } }, checkRes);
